...
def piece_linear(x, slopes, intercepts, breakpoints_x):
    for i in range(1, len(breakpoints_x)):
        if x < 0:
            ...
            return 1
        elif x >= breakpoints_x[i-1] and x < breakpoints_x[i]:
            return slopes[i-1] * x + intercepts[i-1]


# calculates the mass in teh interval for a given distribution
...
def generate_histogram_dist(n, c_tilde, m_tilde, size=1000000):
    ...
    intercepts = [(c_tilde[i-1] * breakpoints_x[i] - c_tilde[i] * breakpoints_x[i-1]) / (breakpoints_x[i] - breakpoints_x[i-1]) for i in range(1, n+2)]
...
def reallocate_mass(points_unif, c_tilde, m_tilde, n):
    ...
    intercepts = [(c_tilde[i-1] * breakpoints[i] - c_tilde[i] * breakpoints[i-1]) / (breakpoints[i] - breakpoints[i-1]) for i in range(1, n+2)]
